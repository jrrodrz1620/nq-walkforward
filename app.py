...
df = df.dropna(subset=["entry_time", "profit_usd"]) if "profit_usd" in df.columns else df
df = df.sort_values("entry_time").reset_index(drop=True)

# Compute profit in points if not present
if "profit_usd" in df.columns and contract_value > 0:
    df["profit_pts"] = df["profit_usd"] / contract_value
...
### What this app does
- Splits your trades into **train/test folds**
