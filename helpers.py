...
def split_folds(df: pd.DataFrame, n: int, train_pct: float, min_trades: int = 5):
...
def gantt_fig(folds):
    ...
    rows = []
    for f in folds:
        rows.append(dict(Fold=f"Fold {f['fold']}", Start=f["train_start"],
                         End=f["train_end"], Phase="Train"))
        rows.append(dict(Fold=f"Fold {f['fold']}", Start=f["test_start"],
                         End=f["test_end"], Phase="Test"))
