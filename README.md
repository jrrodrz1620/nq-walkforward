...
| `demo_breakout_garch.py` | `python demo_breakout_garch.py --out demo/` | **Reproducible trend/breakout example** for `garch_size_review.py`. Generates a regime-switching daily price series (calm trends + high-vol storms) and a Donchian 20/10 breakout trade export, then prints the review command. Shows the verdict *flip* to "vol-targeting helps" — the mirror of the mean-reversion case. Stdlib only. |
